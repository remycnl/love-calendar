<template>
	<div class="min-h-screen">
		...
		<div
			...
			<AdminHeader>
				<template #stats>
					<AdminStats :total="doors.length" :opened="openedCount" />
				</template>
				<template #status>
					<div
						class="hidden sm:flex items-center gap-2 px-3 py-1 rounded-full backdrop-blur-sm"
						style="background: rgba(218, 108, 108, 0.2)">
						<div
							class="w-2 h-2 rounded-full animate-pulse"
							style="background-color: var(--color-coral-300)"></div>
						<span
							class="text-xs font-medium"
							style="color: var(--color-coral-100)"
							>Connecté</span
						>
					</div>
				</template>
				<template #messages>
					<div
						...
						style="
							...
							background: rgba(218, 108, 108, 0.2);
						">
						{{ autoLoginMessage }}
					</div>
					<div
					...
					<div class="flex items-center gap-2 sm:gap-3">
						<NuxtLink
							...
							style="
								...
								box-shadow: 0 4px 15px rgba(218, 108, 108, 0.3);
							">
						...
						<button
							...
							style="
								...
								box-shadow: 0 4px 15px rgba(175, 62, 62, 0.3);
							">
							<span class="text-base sm:text-lg">🚪</span>
			...
			<div class="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 mt-35 mx-auto">
				...
				<div
					...
					<div
						...
						style="
							...
							box-shadow: 0 10px 40px rgba(175, 62, 62, 0.1);
						"
						@mouseenter="
						...
						<div class="relative z-10">
							<div class="flex items-center justify-between mb-3 sm:mb-4">
								...
								<div
									...
									style="background: rgba(175, 62, 62, 0.2)">
									<div
										class="w-2 h-2 sm:w-3 sm:h-3 rounded-full"
										style="background: var(--color-burgundy-500)"></div>
								</div>
							</div>
							<div
								class="text-2xl sm:text-3xl font-black mb-1 sm:mb-2 tracking-tight"
							...
							<div
								class="mt-2 sm:mt-3 h-1 rounded-full"
								style="background: var(--color-burgundy-200)">
								<div
					...
					<div
						class="group relative overflow-hidden backdrop-blur-xl rounded-2xl p-4 sm:p-6 border transition-all duration-500 hover:scale-105"
						style="
							background: rgba(218, 108, 108, 0.08);
							border-color: rgba(218, 108, 108, 0.2);
							box-shadow: 0 10px 40px rgba(218, 108, 108, 0.1);
						"
						@mouseenter="
							$event.currentTarget.style.transform =
								'translateY(-8px) scale(1.02)'
						...
						<div class="relative z-10">
							<div class="flex items-center justify-between mb-3 sm:mb-4">
								...
								<div
									...
									style="background: rgba(218, 108, 108, 0.2)">
									<div
										class="w-2 h-2 sm:w-3 sm:h-3 rounded-full animate-pulse"
										style="background: var(--color-coral-500)"></div>
								</div>
							</div>
							<div
								class="text-2xl sm:text-3xl font-black mb-1 sm:mb-2 tracking-tight"
							...
							<div
								class="mt-2 sm:mt-3 h-1 rounded-full"
								style="background: var(--color-coral-200)">
								<div
									class="h-full rounded-full transition-all duration-1000"
					...
					<div
						class="group relative overflow-hidden backdrop-blur-xl rounded-2xl p-4 sm:p-6 border transition-all duration-500 hover:scale-105"
						style="
							background: rgba(205, 86, 86, 0.08);
							border-color: rgba(205, 86, 86, 0.2);
							box-shadow: 0 10px 40px rgba(205, 86, 86, 0.1);
						"
						@mouseenter="
							$event.currentTarget.style.transform =
								'translateY(-8px) scale(1.02)'
						...
						<div class="relative z-10">
							<div class="flex items-center justify-between mb-3 sm:mb-4">
								...
								<div
									class="w-6 h-6 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center"
									style="background: rgba(205, 86, 86, 0.2)">
									<div
										class="w-2 h-2 sm:w-3 sm:h-3 rounded-full"
										style="background: var(--color-rose-500)"></div>
								</div>
							</div>
							<div
								class="text-2xl sm:text-3xl font-black mb-1 sm:mb-2 tracking-tight"
								style="color: var(--color-rose-700)">
								{{ doors.length - openedCount }}
							</div>
							<div
								class="text-xs sm:text-sm font-medium"
							...
							<div
								class="mt-2 sm:mt-3 h-1 rounded-full"
								style="background: var(--color-rose-200)">
								<div
									class="h-full rounded-full transition-all duration-1000"
									...
									:style="{
										width:
											doors.length > 0
												? ((doors.length - openedCount) / doors.length) * 100 +
												  '%'
												: '0%',
									}"></div>
							</div>
					...
					<div
						...
						style="
							...
							border-color: rgba(175, 62, 62, 0.25);
							box-shadow: 0 10px 40px rgba(175, 62, 62, 0.15);
						"
						@mouseenter="
							$event.currentTarget.style.transform =
								'translateY(-8px) scale(1.02)'
						...
						<div class="relative z-10">
							<div class="flex items-center justify-between mb-3 sm:mb-4">
								...
								<div
									class="w-6 h-6 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center"
									style="background: rgba(175, 62, 62, 0.3)">
									<div
										class="w-2 h-2 sm:w-3 sm:h-3 rounded-full"
										style="background: var(--color-burgundy-600)"></div>
								</div>
							</div>
							<div
								class="text-2xl sm:text-3xl font-black mb-1 sm:mb-2 tracking-tight"
								style="color: var(--color-burgundy-700)">
								{{ Math.round((openedCount / doors.length) * 100) }}%
							</div>
							<div
								class="text-xs sm:text-sm font-medium"
								style="color: var(--color-burgundy-600)">
				...
				<div
					...
					style="
						background: rgba(175, 62, 62, 0.06);
						border-color: rgba(175, 62, 62, 0.15);
						box-shadow: 0 25px 50px rgba(175, 62, 62, 0.1);
					">
					<!-- Header de section avec recherche - responsive -->
					<div
						class="relative p-4 sm:p-6 lg:p-8 border-b backdrop-blur-sm"
						style="
							border-color: rgba(175, 62, 62, 0.1);
							background: linear-gradient(
								135deg,
								rgba(218, 108, 108, 0.05),
								rgba(175, 62, 62, 0.05)
							);
						">
						<div
							class="flex flex-col space-y-4 lg:space-y-0 lg:flex-row lg:items-center lg:justify-between lg:gap-6">
							<div>
								<h2
									class="text-2xl sm:text-3xl font-black tracking-tight mb-2"
								...
								<p
									...
									style="color: var(--color-burgundy-600)">
									Modifiez le contenu et gérez l'état de chaque enveloppe avec
							...
							<div
								...
								<button
									...
									style="
										...
										color: white;
										box-shadow: 0 8px 25px rgba(218, 108, 108, 0.3);
									">
									<span v-if="!isProcessingBulk">{{
										allOpened ? "🔒 Tout Fermer" : "🔓 Tout Ouvrir"
									}}</span>
					...
					<div class="p-4 sm:p-6 lg:p-8">
						<div class="grid gap-4 sm:gap-6 lg:gap-8">
							<DoorAdminCard
								...
								@toggle-status="toggleDoorStatus"
...
<script setup>
import DoorAdminCard from "~/components/DoorAdminCard.vue";
	...
	try {
		...
		if (data.success) {
			...
			setTimeout(async () => {
				...
				showBulkOperationMessage(
					`✅ ${
						actionText.charAt(0).toUpperCase() + actionText.slice(1)
					} terminée: ${data.updatedCount}/${
						data.totalDoors
					} enveloppes mises à jour`,
					4000
				);
			}, 500);
			...
			console.log(
				`Opération terminée: ${data.updatedCount} enveloppes mises à jour sur ${data.totalDoors}`
			);
		} else {
...
.glass-card {
	...
	box-shadow: 0 8px 32px rgba(175, 62, 62, 0.1),
...
.glass-card:hover {
	...
	box-shadow: 0 20px 60px rgba(175, 62, 62, 0.15),
...
.glass-input:focus {
	...
	box-shadow: 0 0 0 3px rgba(218, 108, 108, 0.1),
